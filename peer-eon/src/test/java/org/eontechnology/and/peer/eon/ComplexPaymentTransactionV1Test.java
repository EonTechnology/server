...
public class ComplexPaymentTransactionV1Test extends AbstractTransactionTest {
  ...
  public void illegal_usage() throws Exception {
    ...
    Transaction tx =
        ComplexPaymentBuilder.createNew(
                new Transaction[] {
                  PaymentBuilder.createNew(100L, account.getID())
                      .refBy(new TransactionID(1L))
  ...
  public void invalid_attachment() throws Exception {
    ...
    Transaction nestedTx1 =
        PaymentBuilder.createNew(100L, account.getID()).build(networkID, signer1);
    Transaction nestedTx2 =
        PaymentBuilder.createNew(10L, account1.getID())
  ...
  public void invalid_sequence_multily_head() throws Exception {
    ...
    Transaction nestedTx1 =
        PaymentBuilder.createNew(100L, account.getID()).build(networkID, signer1);
    Transaction nestedTx2 =
        PaymentBuilder.createNew(10L, account.getID()).build(networkID, signer1);
  ...
  public void unable_to_use_lc() throws Exception {
    ...
    Transaction nestedTx3 =
        PaymentBuilder.createNew(1L, account1.getID())
            .refBy(nestedTx1.getID())
            .build(networkID, signer);
  ...
  public void nested_transaction_unknown_type() throws Exception {
    ...
    ISigner s = new Signer("2233445566778899aabbccddeeff00112233445566778899aabbccddeeff0011");
    Transaction nestedTx1 =
        RegistrationBuilder.createNew(s.getPublicKey()).build(networkID, signer1);
    Transaction nestedTx2 =
        PaymentBuilder.createNew(100L, account.getID())
            .refBy(nestedTx1.getID())
            .build(networkID, signer);
  ...
  public void nested_transaction_unknown_type_1() throws Exception {
    ...
    Transaction nestedTx1 =
        PaymentBuilder.createNew(100L, account.getID()).build(networkID, signer1);
    Transaction nestedTx2 =
        RegistrationBuilder.createNew(s.getPublicKey())
            .refBy(nestedTx1.getID())
            .build(networkID, signer);
    Transaction tx =
        ComplexPaymentBuilder.createNew(new Transaction[] {nestedTx1, nestedTx2})
  ...
  public void nested_transaction_invalid_recipient() throws Exception {
    ...
    Transaction nestedTx1 =
        PaymentBuilder.createNew(100L, account.getID()).build(networkID, signer1);
    Transaction nestedTx2 =
        ColoredPaymentBuilder.createNew(
                10L, new ColoredCoinID(account.getID()), new AccountID(s.getPublicKey()))
            .refBy(nestedTx1.getID())
            .build(networkID, signer);
    Transaction tx =
        ComplexPaymentBuilder.createNew(new Transaction[] {nestedTx1, nestedTx2})
  ...
  public void success() throws Exception {
    ...
    account1 =
        AccountProperties.setProperty(
            account1, new RegistrationDataProperty(signer1.getPublicKey()));
    ...
    // run
    Transaction nestedTx1 =
        PaymentBuilder.createNew(100L, account.getID()).forFee(0L).build(networkID, signer1);
    Transaction nestedTx2 =
        ColoredPaymentBuilder.createNew(10L, new ColoredCoinID(account.getID()), account1.getID())
            .forFee(0L)
            .refBy(nestedTx1.getID())
    ...
    Assert.assertEquals(
        AccountProperties.getColoredBalance(updatedAccount).getBalance(new ColoredCoinID(id)),
        990L);
    ...
    Assert.assertEquals(
        AccountProperties.getColoredBalance(updatedAccount1).getBalance(new ColoredCoinID(id)),
        10L);
  }
}
