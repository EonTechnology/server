...
import org.eontechnology.and.peer.eon.midleware.parsers.ComplexPaymentParserV3;
...
public class ComplexPaymentTransactionV3Test extends AbstractTransactionTest {
  ...
  public void success() throws Exception {
    ...
    Transaction nestedTx1 =
        PaymentBuilder.createNew(AMOUNT, account3)
            .forFee(0L)
            .payedBy(account1)
            .build(networkID, signer2);
    Transaction nestedTx2 =
        ColoredPaymentBuilder.createNew(C_AMOUNT, new ColoredCoinID(account3), account2)
            .forFee(0L)
            .refBy(nestedTx1.getID())
            .build(networkID, signer3);
    Transaction nestedTx3 =
        PaymentBuilder.createNew(DELIVERY, account2)
            .forFee(0L)
            .refBy(nestedTx1.getID())
            .build(networkID, signer3);

    Transaction tx =
        ComplexPaymentBuilder.createNew(new Transaction[] {nestedTx1, nestedTx2, nestedTx3})
            .forFee(FEE)
    ...
    Assert.assertEquals(
        SC_MONEY, AccountProperties.getColoredBalance(acc).getBalance(new ColoredCoinID(account1)));
    Assert.assertEquals(
        0L, AccountProperties.getColoredBalance(acc).getBalance(new ColoredCoinID(account2)));
    Assert.assertEquals(
        0L, AccountProperties.getColoredBalance(acc).getBalance(new ColoredCoinID(account3)));

    acc = ledger.getAccount(account2);
    Assert.assertEquals(S_MONEY - AMOUNT + DELIVERY, AccountProperties.getBalance(acc).getValue());
    Assert.assertEquals(
        0L, AccountProperties.getColoredBalance(acc).getBalance(new ColoredCoinID(account1)));
    Assert.assertEquals(
        SC_MONEY, AccountProperties.getColoredBalance(acc).getBalance(new ColoredCoinID(account2)));
    Assert.assertEquals(
        C_AMOUNT, AccountProperties.getColoredBalance(acc).getBalance(new ColoredCoinID(account3)));
  ...
  public void success_4() throws Exception {
    ...
    Transaction tx =
        ComplexPaymentBuilder.createNew(new Transaction[] {nestedTx1})
            .forFee(FEE)
            .build(networkID, signer1);
    ...
    Assert.assertEquals(
        0L, AccountProperties.getColoredBalance(acc).getBalance(new ColoredCoinID(account2)));
    Assert.assertEquals(
        SC_MONEY, AccountProperties.getColoredBalance(acc).getBalance(new ColoredCoinID(account3)));
  }
  ...
  public void success_6() throws Exception {
    ...
    tx =
        ComplexPaymentBuilder.createNew(txSet.toArray(new Transaction[] {}))
            .forFee(FEE)
            .build(networkID, signer1);
  ...
  public void error_no_payer() throws Exception {
    ...
    Transaction nestedTx1 =
        PaymentBuilder.createNew(AMOUNT, account3).forFee(0L).build(networkID, signer2);
