...
public class ColoredCoinPaymentTransactionTest extends AbstractTransactionTest {
  ...
  public void setUp() throws Exception {
    ...
    AccountProperties.setProperty(
        recipientAccount, new RegistrationDataProperty(recipient.getPublicKey()));
  ...
  public void invalid_attach() throws Exception {
    ...
    Transaction tx =
        new TransactionBuilder(TransactionType.ColoredCoinPayment, map).build(networkID, sender);
    validate(parser, tx);
  }
  ...
  public void unknown_color() throws Exception {
    ...
    Transaction tx =
        ColoredPaymentBuilder.createNew(
                9999L, new ColoredCoinID(coloredCoinAccount.getID()), recipientAccount.getID())
  ...
  public void not_enough_colored_coun() throws Exception {
    ...
    Transaction tx =
        ColoredPaymentBuilder.createNew(
                10001L, new ColoredCoinID(coloredCoinAccount.getID()), recipientAccount.getID())
            .build(networkID, sender);
    validate(parser, tx);
  }
  ...
  public void success_auto_emission() throws Exception {
    ...
    Transaction tx =
        ColoredPaymentBuilder.createNew(
                2000L, new ColoredCoinID(coloredCoinAccount.getID()), recipientAccount.getID())
            .build(networkID, s);
    validate(parser, tx);
    Assert.assertEquals(
        12000L, AccountProperties.getColoredCoin(coloredCoinAccount).getMoneySupply());
  }
  ...
  public void success_auto_withdrawl() throws Exception {
    ...
    Transaction tx =
        ColoredPaymentBuilder.createNew(
                10000L, new ColoredCoinID(coloredCoinAccount.getID()), coloredCoinAccount.getID())
            .build(networkID, sender);
    validate(parser, tx);
