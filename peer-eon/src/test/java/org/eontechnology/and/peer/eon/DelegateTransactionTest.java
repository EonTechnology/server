...
public class DelegateTransactionTest extends AbstractTransactionTest {
  ...
  public void delegates_limit() throws Exception {
    ...
    int i = 0;
    while (validationMode.delegatesEntrySet().size()
        < Constant.TRANSACTION_CONFIRMATIONS_MAX_SIZE) {
  ...
  public void delegates_pre_limit() throws Exception {
    ...
    AccountProperties.setProperty(
        delegateAccount2, new RegistrationDataProperty(delegate_2.getPublicKey()));

    ledger.putAccount(delegateAccount2);

    int i = 0;
    while (validationMode.delegatesEntrySet().size()
        < Constant.TRANSACTION_CONFIRMATIONS_MAX_SIZE - 1) {
      validationMode.setWeightForAccount(new AccountID(i++), 50);
    }
  ...
  public void exceed_vote_polls_limit() throws Exception {
    ...
    Transaction tx =
        DelegateBuilder.createNew(delegateAccount1.getID(), 40).build(networkID, sender);
    validate(parser, tx);
  }
