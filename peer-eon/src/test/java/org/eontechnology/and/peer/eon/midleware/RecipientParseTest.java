...
public class RecipientParseTest {
  ...
  public void colored_payment() throws Exception {
    Transaction tx =
        ColoredPaymentBuilder.createNew(
                9999L, new ColoredCoinID(1L), new AccountID(signer.getPublicKey()))
            .build(networkID, signer);

    ColoredCoinPaymentParser parser = new ColoredCoinPaymentParser();
    Assert.assertEquals(
        parser.getDependencies(tx), Collections.singleton(new AccountID(signer.getPublicKey())));
  }
  ...
  public void complex_payment_v1() throws Exception {
    ...
    Transaction nestedTx1 =
        PaymentBuilder.createNew(100L, new AccountID(signer.getPublicKey()))
            .forFee(0L)
            .build(networkID, signer1);
    Transaction nestedTx2 =
        ColoredPaymentBuilder.createNew(
                10L, new ColoredCoinID(0L), new AccountID(signer1.getPublicKey()))
            .refBy(nestedTx1.getID())
            .build(networkID, signer);
    Transaction nestedTx3 =
        PaymentBuilder.createNew(1L, new AccountID(signer1.getPublicKey()))
            .refBy(nestedTx1.getID())
            .build(networkID, signer);
    Transaction tx =
    ...
    Assert.assertEquals(
        parser.getDependencies(tx), Collections.singleton(new AccountID(signer1.getPublicKey())));
  }
  ...
  public void complex_payment_error_v1() throws Exception {
    ...
    Transaction nestedTx3 =
        PaymentBuilder.createNew(1L, new AccountID(signer1.getPublicKey()))
            ...
            .build(networkID, signer);
    Transaction tx =
        ComplexPaymentBuilder.createNew(new Transaction[] {nestedTx2, nestedTx3})
            .build(networkID, signer);
  ...
  public void complex_payment_v2() throws Exception {
    ...
    Transaction nestedTx1 =
        PaymentBuilder.createNew(100L, new AccountID(signer3.getPublicKey()))
            .forFee(0L)
            .payedBy(new AccountID(signer1.getPublicKey()))
            .build(networkID, signer2);
    Transaction nestedTx2 =
        PaymentBuilder.createNew(100L, new AccountID(signer4.getPublicKey()))
            .forFee(0L)
            .refBy(nestedTx1.getID())
            .build(networkID, signer2);
    Transaction tx =
  ...
  public void registration() throws Exception {
    ...
    Transaction tx =
        RegistrationBuilder.createNew(newSigner.getPublicKey()).build(networkID, signer);
    RegistrationParser parser = new RegistrationParser();
    Assert.assertEquals(
        parser.getDependencies(tx), Collections.singleton(new AccountID(newSigner.getPublicKey())));
  }
