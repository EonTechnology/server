 ...
 *
 * <p>Used to storage the state and to describe configuration of a services env.
 */
public class PeerInfo implements Comparable<PeerInfo> {

  /**
   * The env has an unknown state. 1. The initial state of the env. 2. In this state the env gets
   * after exclusion from the black list.
   */
  public static final int STATE_AMBIGUOUS = 10;
  /** The env is in the pool of active nodes (used to synchronize data) */
  public static final int STATE_CONNECTED = 20;
  /** In this state, the env falls if it does not respond, or use unsupported protocol. */
  public static final int STATE_DISCONNECTED = 30;

  /** Peer from init list */
  public static final int TYPE_IMMUTABLE = 10;
  /** Peer added from other peer */
  public static final int TYPE_NORMAL = 20;
  /** Inner peer */
  public static final int TYPE_INNER = 30;
   ...
   *
   * @return true if the host address not be distributed over the network, otherwise- false
   */
  ...
  /**
   * Returns the characteristics of the services env for identification, search for provided
   * services, etc.
   *
   * @return
   */
