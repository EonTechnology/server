 ...
 *
 * <p>Pre-validation of transactions before putting in Backlog.
 *
 * @see IBacklog
 */
public class Backlog implements IBacklog {
  ...
  private void InitContext(Block block) {
    int timestamp = fork.getNextBlockTimestamp(block.getTimestamp());

    ITimeProvider blockTimeProvider = new ImmutableTimeProvider(timestamp);
    ITimeProvider peerTimeProvider =
        new ITimeProvider() {
          @Override
          public int get() {
    ...
    transactionValidator =
        transactionValidatorFabric.getAllValidators(blockTimeProvider, peerTimeProvider);
  }
