...
/**
 * Performs the task of synchronizing the list of unconfirmed transactions of the current node and
 * random connected node
 *
 * <p>Synchronization of transactions is performed only if the node is in the actual state, i.e. the
 * current hard-fork (see {@link IFork}) time has not expired. In the first step, a random connected
 * node for interaction is selected. The randomly node selection algorithm is described in {@code
 * ExecutionContext} implementation (see {@link ExecutionContext#getAnyConnectedPeer}). Next, a list
 * of transactions, which falls into the next block on the current node, is sent to the services
 * node (which selected on first step). The services node compares the resulting transaction list to
 * its own and returns the missing transactions. At the end, the received transactions are added to
 * the Backlog (see {@link IBacklog }) list.
 */
public final class SyncTransactionListTask implements Runnable {
  ...
  public SyncTransactionListTask(
      IFork fork,
    ...
    try {
      ...
      if (peer != null) {
        ...
        try {
          ...
          Loggers.trace(
              SyncTransactionListTask.class, "Failed to execute a request. Target: " + peer, e);
