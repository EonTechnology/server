 ...
 *
 * <p>Attempts to connect a random unconnected node. A node must belong to the same generation of
 * hard-forks and a instance of the network as the current feast. Connection is carried out for a
 * period of time, the duration of which is defined in the context. The algorithm for selecting the
 * peer for connection and the duration is described in {@code ExecutionContext}.
 */
  ...
  public void run() {
    ...
    try {
      ...
      if (peer != null) {
        ...
        if (forkNumber != remoteAttributes.getFork()
            && forkNumberReal != remoteAttributes.getFork()) {
        ...
        PeerInfo.Metadata metadata =
            new PeerInfo.Metadata(
                remoteAttributes.getPeerId(),
