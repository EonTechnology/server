...
public class BlockchainProvider implements IBlockchainProvider {
  ...
  public BlockchainProvider(Storage storage, BlockchainEventManager blockchainEventManager)
      throws SQLException {
  ...
  protected void setPointerTo(Block newHead) {
    ...
    List<DbTransaction> forked =
        storage.callInTransaction(
            new Callable<List<DbTransaction>>() {
              @Override
  ...
  private BlockID intersect(Block aBlock, Block bBlock) throws SQLException {
    ...
    if (Math.abs(aBlock.getTimestamp() - bBlock.getTimestamp()) > Constant.SECONDS_IN_DAY) {
    ...
    int aBlockHeight = aBlock.getHeight();
    int bBlockHeight = bBlock.getHeight();
