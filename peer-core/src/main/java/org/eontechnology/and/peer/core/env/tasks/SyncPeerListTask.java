 ...
 *
 * <p>At the fist step, a randomly node is selected ( {@link ExecutionContext#getAnyConnectedPeer}).
 * Next, task is requested to list the nodes connected by the services node. After, the resulting
 * list is added to the known address pool on the current node.
 */
          ...
          for (String address : addresses) {
            ...
            if (address.length() > 0) {
              ...
              if (context.getPeers().getPeerByIP(address) == null) {
                ...
                Loggers.info(
                    SyncPeerListTask.class, "Added new peer \"{}\".", newPeer.getAddress());
              }
            }
          }
        }
