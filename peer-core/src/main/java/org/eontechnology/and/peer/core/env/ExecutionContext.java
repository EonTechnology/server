...
public class ExecutionContext {
  ...
  public Peer getAnyConnectedPeer() {
    ...
    PeerInfo pi =
        getAnyPeer(
            new PeerBasePredicate(getHost().getPeerID(), isInnerPeersUsing) {
  ...
  /**
   * Initiate an event that indicates that the chain of blocks state is the same as the specified
   * remote env.
   *
  ...
  public void raiseSynchronizedEvent(Object source, Peer peer) {
    peerEventSupport.raiseEvent(
        new DispatchableEvent<IPeerEventListener, PeerEvent>(new PeerEvent(source, peer)) {
          @Override
    ...
    int peerCount =
        peers.count(
            new Predicate<PeerInfo>() {
              ...
              @Override
              public boolean test(PeerInfo peer) {
                return peer.getState() == PeerInfo.STATE_CONNECTED
