...
/**
 * Performs the task of removing expired transaction, duplicate transactions, transactions with
 * invalid signatures, etc.
 *
 * <p>In other words, re-import of transactions taking into account the new state.
 */
public class BacklogCleaner implements IBlockchainEventListener {
  ...
  private SortedSet<Transaction> set =
      Collections.synchronizedSortedSet(new TreeSet<>(new TransactionComparator()));
      ...
      try {
        while (!Thread.interrupted() && !set.isEmpty()) {
          ...
          try {
            ...
            Loggers.info(
                BacklogCleaner.class,
                String.format(
                    "ValidationError %s: %s (%s)",
                    first.getID(), e.getMessage(), first.toString()));
          }
        }
      } catch (Throwable th) {
