...
public class Blockchain {
  ...
  private HashMap<BlockID, Integer> timestampCache = new HashMap<>();
  ...
  private QueryBuilder<DbBlock, Long> timestampBlockIdQueryBuilder = null;
  ...
  public int getBlockTimestamp(BlockID id) throws SQLException {

    Integer timestamp = timestampCache.get(id);
    if (timestamp == null) {

      if (timestampBlockIdQueryBuilder == null) {
        timestampBlockIdQueryBuilder = daoBlocks.queryBuilder();
        timestampBlockIdQueryBuilder.selectColumns("timestamp");
        timestampBlockIdQueryBuilder.where().eq("id", vID);
      }

      vID.setValue(id.getValue());
      DbBlock dbBlock = timestampBlockIdQueryBuilder.queryForFirst();
      if (dbBlock == null) {
        return 0;
      }
      timestamp = dbBlock.getTimestamp();

      timestampCache.put(id, timestamp);
    }
