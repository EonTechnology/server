...
public class LedgerProvider {
  ...
  public LedgerProvider(Storage storage, IFork fork) {
  ...
  public ILedger getLedger(Block block) {
    return getLedger(block.getSnapshot(), fork.getNextBlockTimestamp(block.getTimestamp()));
  }

  public ILedger getLedger(String snapshot, int timestamp) {

    BufferedTreeNodeCollection collection =
        new BufferedTreeNodeCollection(new CachedNodeCollection(storage));
    ...
    Function<StateTree<Account>, Void> callback =
        new Function<StateTree<Account>, Void>() {
  ...
  private static class AccountConverter implements ITreeNodeConverter<Account> {
    ...
    public Account convert(TreeNode treeNode) {
      ...
      if (map.containsKey("properties")) {
        ...
        return new Account(
            new AccountID(map.get("id").toString()), properties.toArray(new AccountProperty[0]));
      }
