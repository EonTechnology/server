...
public interface ITransactionSynchronizationService {
  ...
  /**
   * Returns the list of Unconfirmed Transactions (currently not included in the block), except for
   * the transactions which are specified in parameters.
   *
   ...
   * @throws RemotePeerException An error in the protocol. Invalid parameters are passed or an error
   *     in the response format.
   * @throws IOException Error during access to the env.
   */
  Transaction[] getTransactions(String lastBlockId, String[] ignoreList)
