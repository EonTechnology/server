...
import org.eontechnology.and.peer.core.ledger.storage.DbNodeHelper;
import org.eontechnology.and.peer.core.ledger.tree.ITreeNodeCollection;
    ...
    try {
      ...
      if (dbn != null) {
        if (dbn.getTimestamp() < node.getTimestamp()) {
          if (dbNodeHelper.updateTimestamp(
              node.getID().getKey(), node.getID().getIndex(), node.getTimestamp())) {
