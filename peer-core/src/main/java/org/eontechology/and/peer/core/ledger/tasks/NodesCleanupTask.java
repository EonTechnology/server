package org.eontechology.and.peer.core.ledger.tasks;
...
public class NodesCleanupTask implements Runnable {
    ...
    public void run() {
        ...
        try {
            ...
            long lastBlockID = Long.parseLong(dao.queryRaw("select value from settings where name = 'LastBlockId'")
                                                 .getFirstResult()[0]);
            int blockTime = Integer.parseInt(dao.queryRaw("select timestamp from blocks where id = ?",
                                                          Long.toString(lastBlockID)).getFirstResult()[0]);
            int timestamp = blockTime - (2 * Constant.SECONDS_IN_DAY);

            Loggers.trace(NodesCleanupTask.class, "Beginning colorizing. Last block id '{}'", new BlockID(lastBlockID));
    ...
    private void step_2_colorizeTree_recursive(int timestamp) throws SQLException, InterruptedException {

        String[] firstResult =
                dao.queryRaw("select snapshot from blocks where timestamp = ? and tag = 1", Integer.toString(timestamp))
                   .getFirstResult();
        if (firstResult == null) {
            return;
        }
    ...
    private void step_3_colorizeTree_sql(int timestamp) throws SQLException, InterruptedException {
        ...
        int updated = dao.updateRaw("update nodes set color = 1 where color = 0 and timestamp > ?",
                                    Integer.toString(timestamp));

        updated += dao.updateRaw(
