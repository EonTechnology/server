 ...
 *
 * @param <TListener>
 *            Listener interface specification. Defines the list of events.
 */
public class Dispatcher<TListener extends EventListener> extends DispatchableEventDispatcher<TListener> {

	final Collection<ExecutableListener<TListener>> listeners = new ArrayList<ExecutableListener<TListener>>();
	...
	public synchronized void addListener(TListener listener, Executor executor) {
		listeners.add(new ExecutableListener<TListener>(listener, executor));
	}
	...
	public synchronized void removeListener(TListener listener, Executor executor) {
		listeners.remove(new ExecutableListener<TListener>(listener, executor));
	}
	...
	private synchronized Collection<ExecutableListener<TListener>> getListeners() {
		Collection<ExecutableListener<TListener>> listeners = new ArrayList<ExecutableListener<TListener>>(
				this.listeners.size());
	...
	private static class ExecutableListener<TListener extends EventListener> {
		...
		public boolean equals(Object other) {
			...
			return (getExecutor().equals(listener.getExecutor()) && getListener().equals(listener.getListener()));
		}
	}
}
