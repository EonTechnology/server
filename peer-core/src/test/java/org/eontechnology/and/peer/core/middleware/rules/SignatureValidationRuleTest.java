...
public class SignatureValidationRuleTest extends AbstractValidationRuleTest {
  ...
  public void setUp() throws Exception {
    ...
    Account delegate1Account =
        Mockito.spy(new TestAccount(new AccountID(delegate1.getPublicKey())));
    ledger.putAccount(delegate1Account);

    Account delegate2Account =
        Mockito.spy(new TestAccount(new AccountID(delegate2.getPublicKey())));
  ...
  public void delegate_unknown() throws Exception {
    ...
    Mockito.when(ledger.getAccount(ArgumentMatchers.any()))
        .thenAnswer(
            new Answer<Account>() {
              @Override
  ...
  public void duplicate_confirmation() throws Exception {
    ...
    Transaction tx =
        Builder.newTransaction(timeProvider).build(networkID, sender, new ISigner[] {sender});
    validate(tx);
  }
