...
public class ConfirmationsSetValidationRuleTest extends AbstractValidationRuleTest {
    ...
    public void mfa_allow_payer() throws Exception {
        ...
        Transaction tx =
                Builder.newTransaction(timeProvider).payedBy(id).build(networkID, sender, new ISigner[] {unknown});
        validate(tx);
    }
    ...
    public void unset_mfa_allow_payer() throws Exception {
        ...
        Transaction tx = Builder.newTransaction(timeProvider).payedBy(id).build(networkID, delegate1, new ISigner[] {
                sender
        });
        validate(tx);
    }
