...
public class PayerTestIT {
    ...
    private static final String SENDER = "9e641020d3803008bf4e8a15ad05f84fb8eb3220037322ebc5fa58b70c3f1bd1";
    ...
    public void step_1() throws Exception {
        ...
        Assert.assertNotEquals("Block generated", lastBlockID, ctx.blockExplorerService.getLastBlock().getID());
        ...
        Transaction tx3 = PaymentBuilder.createNew(amount, delegate_1_id)
                                        .forFee(fee)
        ...
        Transaction tx4 = PaymentBuilder.createNew(amount, delegate_2_id)
        ...
        Assert.assertEquals("Blocks synchronized",
                            ctx.blockchain.getLastBlock().getID(),
                            ctx2.blockchain.getLastBlock().getID());

        // check tx history
        assertEquals("Delegate1 tx count", 3, ctx2.transactionExplorerService.getByAccountId(delegate_1_id, 0).size());

        assertEquals("Delegate2 tx count", 2, ctx2.transactionExplorerService.getByAccountId(delegate_2_id, 0).size());

        assertEquals("Sender tx count", 2, ctx2.transactionExplorerService.getByAccountId(senderID, 0).size());
    }
}
