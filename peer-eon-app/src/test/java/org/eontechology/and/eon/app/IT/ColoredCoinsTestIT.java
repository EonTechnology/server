...
import org.eontechology.and.eon.app.api.bot.ColoredCoinBotService;
...
import org.eontechology.and.peer.eon.midleware.parsers.ColoredCoinPaymentParser;
...
import org.eontechology.and.peer.eon.midleware.parsers.ColoredCoinRemoveParser;
import org.eontechology.and.peer.eon.midleware.parsers.ColoredCoinSupplyParserV1;
import org.eontechology.and.peer.eon.midleware.parsers.ColoredCoinSupplyParserV2;
...
import org.eontechology.and.peer.tx.midleware.builders.ColoredCoinRemoveBuilder;
...
public class ColoredCoinsTestIT {
    ...
    public static class V1 {
        ...
        public void step_1_new_colored_coin() throws Exception {
            ...
            Assert.assertEquals(balance.coloredCoins.get(new ColoredCoinID(accountID).toString()),
                                Long.valueOf(10000L));
        }
    ...
    public static class V2 {
        ...
        public void setUp() throws Exception {
            ...
            ctx = new PeerContext(PeerStarterFactory.create()
                                                    ...
                                                    .route(TransactionType.ColoredCoinPayment,
                                                           new ColoredCoinPaymentParser())
                                                    .route(TransactionType.ColoredCoinSupply,
                                                           new ColoredCoinSupplyParserV2())
                                                    .route(TransactionType.ColoredCoinRemove,
                                                           new ColoredCoinRemoveParser())
                                                    .seed(GENERATOR)
                                                    .build(mockTimeProvider));
            coloredCoinSigner = new TestSigner(COLORED_COIN);
        ...
        public void create_colored_coin() throws Exception {
            ...
            Transaction tx1 = ColoredCoinRegistrationBuilder.createNew(10000L, 1)
                                                            .validity(timestamp, 3600)
                                                            .build(ctx.getNetworkID(), coloredCoinSigner);
            ctx.transactionBotService.putTransaction(tx1);
            ctx.generateBlockForNow();
            ...
            AccountID senderID = new AccountID(coloredCoinSigner.getPublicKey());
            AccountID recipientID = new AccountID(ctx.getSigner().getPublicKey());
            Transaction tx2 = ColoredPaymentBuilder.createNew(4000L, new ColoredCoinID(senderID), recipientID)
                                                   .validity(timestamp, 60 * 60)
                                                   .build(ctx.getNetworkID(), coloredCoinSigner);
            ctx.transactionBotService.putTransaction(tx2);

            timestamp = ctx.blockExplorerService.getLastBlock().getTimestamp() + Constant.BLOCK_PERIOD;
            Mockito.when(mockTimeProvider.get()).thenReturn(timestamp + 1);
            ctx.generateBlockForNow();

            ColoredCoinID id = new ColoredCoinID(senderID);
            AccountBotService.EONBalance senderBalance = ctx.accountBotService.getBalance(senderID.toString());
            AccountBotService.EONBalance recipientBalance = ctx.accountBotService.getBalance(recipientID.toString());
        ...
        public void create_colored_coin_auto_emission_mode() throws Exception {
            ...
            Transaction tx1 = ColoredCoinRegistrationBuilder.createNew(1)
                                                            .validity(timestamp, 3600)
                                                            .build(ctx.getNetworkID(), coloredCoinSigner);
            ctx.transactionBotService.putTransaction(tx1);
            ctx.generateBlockForNow();
        ...
        public void delete_colored_coin_preset_mode() throws Exception {
            ...
            int timestamp = ctx.blockExplorerService.getLastBlock().getTimestamp() + Constant.BLOCK_PERIOD;
            Mockito.when(mockTimeProvider.get()).thenReturn(timestamp + 1);
            ctx.generateBlockForNow();
            ...
            Transaction tx2 = ColoredCoinRemoveBuilder.createNew()
                                                      .validity(mockTimeProvider.get(), 3600)
                                                      .build(ctx.getNetworkID(), coloredCoinSigner);
            ctx.transactionBotService.putTransaction(tx2);
        ...
        public void toggling_to_auto_emission_mode() throws Exception {
            ...
            Transaction tx3 = ColoredCoinSupplyBuilder.createNew(ColoredCoinEmitMode.AUTO.toString())
                                                      .validity(mockTimeProvider.get(), 60 * 60)
                                                      .build(ctx.getNetworkID(), coloredCoinSigner);
            ctx.transactionBotService.putTransaction(tx3);
            timestamp = ctx.blockExplorerService.getLastBlock().getTimestamp() + Constant.BLOCK_PERIOD;
        ...
        public void toggling_to_preset_mode() throws Exception {
            ...
            Transaction tx3 = ColoredCoinSupplyBuilder.createNew(10000)
                                                      .validity(mockTimeProvider.get(), 60 * 60)
                                                      .build(ctx.getNetworkID(), coloredCoinSigner);
            ctx.transactionBotService.putTransaction(tx3);
            timestamp = ctx.blockExplorerService.getLastBlock().getTimestamp() + Constant.BLOCK_PERIOD;
        ...
        public void change_money_supply() throws Exception {
            ...
            Transaction tx2 = ColoredCoinSupplyBuilder.createNew(6000)
                                                      .validity(mockTimeProvider.get(), 60 * 60)
                                                      .build(ctx.getNetworkID(), coloredCoinSigner);
            ctx.transactionBotService.putTransaction(tx2);
            timestamp = ctx.blockExplorerService.getLastBlock().getTimestamp() + Constant.BLOCK_PERIOD;
