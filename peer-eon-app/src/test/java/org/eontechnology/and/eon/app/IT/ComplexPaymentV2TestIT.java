...
public class ComplexPaymentV2TestIT {
    ...
    ctx =
        new PeerContext(
            PeerStarterFactory.create()
                ...
                .route(
                    TransactionType.ColoredCoinRegistration, new ColoredCoinRegistrationParserV2())
  ...
  public void step1_complex_payment() throws Exception {
    ...
    AccountBotService.EONBalance accountBalance =
        ctx.accountBotService.getBalance(accountID.toString());
    AccountBotService.EONBalance newAccountBalance =
        ctx.accountBotService.getBalance(newAccountID.toString());
    ...
    Transaction tx3 =
        ColoredCoinRegistrationBuilder.createNew(1000L, 1)
            .validity(timeProvider.get(), 3600)
    ...
    Assert.assertEquals(
        newAccountBalance.coloredCoins.get((new ColoredCoinID(newAccountID)).toString()),
        (Long) 1000L);
    ...
    Transaction nestedTx2 =
        ColoredPaymentBuilder.createNew(
                10L, new ColoredCoinID(newAccountID), new AccountID(ctx.getSigner().getPublicKey()))
            .validity(timeProvider.get() - 1, 3600)
            .forFee(0L)
    ...
    Transaction nestedTx4 =
        PaymentBuilder.createNew(1L, accountID)
            ...
            .build(ctx.getNetworkID(), newAccountSigner);
    Transaction tx5 =
        ComplexPaymentBuilder.createNew(new Transaction[] {nestedTx1, nestedTx4})
            .validity(timeProvider.get(), 3600)
            .forFee(30)
            .build(ctx.getNetworkID(), newAccountSigner);
    try {
      ...
      Assert.assertEquals(
          "Invalid sequence. Transaction already exist.", ex.getCause().getMessage());
    }
    ...
    Assert.assertEquals(
        accountBalance.coloredCoins.get((new ColoredCoinID(newAccountID)).toString()), (Long) 10L);
    newAccountBalance = ctx.accountBotService.getBalance(newAccountID.toString());
