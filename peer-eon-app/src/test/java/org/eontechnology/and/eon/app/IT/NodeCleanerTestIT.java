...
public class NodeCleanerTestIT {
  ...
  private static String GENERATOR1 =
  ...
  public void step_1_cleaning() throws Exception {
    ...
    int timestamp = lastBlock.getTimestamp() + Constant.SECONDS_IN_DAY + 1;
    ...
    timestamp = lastBlock.getTimestamp() + 2 * Constant.SECONDS_IN_DAY + 180 + 1;
    Mockito.when(mockTimeProvider1.get()).thenReturn(timestamp);
    Mockito.when(mockTimeProvider2.get()).thenReturn(timestamp);
    ...
    Dao<DbNode, Long> nodeDao1 =
        DaoManager.createDao(ctx1.storage.getConnectionSource(), DbNode.class);
    Dao<DbNode, Long> nodeDao2 =
        DaoManager.createDao(ctx2.storage.getConnectionSource(), DbNode.class);
  ...
  private void checkContext(PeerContext ctx) throws Exception {
    for (String seed : seeds) {
      AccountBotService.State state =
          ctx.accountBotService.getState(
              new AccountID(new TestSigner(seed).getPublicKey()).toString());
  ...
  private void changeState(Random random) throws Exception {
    ...
    AccountID recipient =
        new AccountID(new TestSigner(seeds[random.nextInt(seeds.length - 1)]).getPublicKey());

    ISigner signer = new TestSigner(seeds[random.nextInt(seeds.length - 1)]);
    Transaction tx =
        PaymentBuilder.createNew(100L, recipient)
