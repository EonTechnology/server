...
public class Utils {
  ...
  public static void comparePeer(PeerContext ctx1, PeerContext ctx2) throws Exception {
    ...
    while (lastBlockID.getValue() != 0) {
      ...
      Assert.assertEquals(
          Format.convert(block.getGenerationSignature()),
          Format.convert(blockNew.getGenerationSignature()));
      Assert.assertEquals(block.getSenderID(), blockNew.getSenderID());
      Assert.assertEquals(
          Format.convert(block.getSignature()), Format.convert(blockNew.getSignature()));
      ...
      for (int i = 0; i < transactions.length; i++) {
        ...
        Assert.assertEquals(
            Format.convert(bencode.encode(transaction.getData())),
            Format.convert(bencode.encode(transactionNew.getData())));
        Assert.assertEquals(
            Format.convert(transaction.getSignature()),
  ...
  public String getGenesisBlockAsJSON(Block lastBlock, LedgerProvider ledgerProvider)
      throws Exception {
