...
public class InstanceConnectTestIT {
  ...
  public void step_1_sync_service() throws Exception {
    ...
    PeerContext ctx_1 =
        new PeerContext(PeerStarterFactory.create().seed(GENERATOR_1).build(mockTimeProvider));
    PeerContext ctx_2 =
        new PeerContext(PeerStarterFactory.create().seed(GENERATOR_2).build(mockTimeProvider));
    PeerContext ctx_3 =
        new PeerContext(PeerStarterFactory.create().seed(GENERATOR_3).build(mockTimeProvider));
    ...
    Assert.assertEquals(
        "2=>3) Sync all connected peers", 2, ctx_3.context.getPeers().getPeersList().length);
  }
  ...
  public void step_4_remove_connected() throws Exception {
    ...
    Assert.assertTrue(
        "IInstance is disconnected", peer.getPeerInfo().getState() == PeerInfo.STATE_DISCONNECTED);
    ...
    Assert.assertTrue(
        "IInstance is connected", peer.getPeerInfo().getState() == PeerInfo.STATE_CONNECTED);
  }
  ...
  public void step_5_remove_blacklisted() throws Exception {
    ...
    ctx_1.context.blacklistPeer(
        peer, System.currentTimeMillis() - ctx_1.context.getBlacklistingPeriod() - 1);
