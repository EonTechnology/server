...
public class TransactionNetworkTestIT {
  ...
  public void step_2_syncTran() throws Exception {
    ...
    Assert.assertNotNull(
        "Transaction accepted in (1)", ctx1.backlogExplorerService.get(tx.getID()));
    Assert.assertNull(
        "Transaction not accepted in (2)", ctx2.backlogExplorerService.get(tx.getID()));
  }
  ...
  public void step_3_syncBlock() throws Exception {
    ...
    Assert.assertNotNull(
        "Transaction accepted in (1)", ctx1.transactionExplorerService.getById(tx.getID()));
    Assert.assertNotEquals(
        "Blockchain not synchronized (2)", lastBlock1.getID(), lastBlock2.getID());
  }
}
