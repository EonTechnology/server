...
public class AddPeerTestIT {
  ...
  public void step_1_OK() throws IOException, RemotePeerException {
    ...
    try {
      Assert.assertFalse(
          ctx1.syncMetadataPeerService.addPeer(ctx1.context.getHost().getPeerID(), "123"));
      Assert.assertTrue(false);
    } catch (Exception ex) {
    ...
    // All OK
    Assert.assertTrue(
        ctx1.syncMetadataPeerService.addPeer(ctx2.context.getHost().getPeerID(), "123"));
    ...
    try {
      Assert.assertFalse(
          ctx1.syncMetadataPeerService.addPeer(ctx2.context.getHost().getPeerID(), "123:8888"));
      Assert.assertTrue(false);
    } catch (Exception ex) {
