...
public class BlockSyncTestIT {
  ...
  private static String GENERATOR2 =
  ...
  public void step_1_one_block() throws Exception {
    ...
    Mockito.verify(ctx1.syncBlockPeerService, Mockito.times(0))
        .getBlockHistory(ArgumentMatchers.any());
  ...
  public void step_2_two_block() throws Exception {
    ...
    Mockito.verify(ctx1.syncBlockPeerService, Mockito.times(1)).getLastBlock();
    Mockito.verify(ctx1.syncBlockPeerService, Mockito.times(1))
        .getBlockHistory(ArgumentMatchers.any());
  ...
  public void step_3_replace_generated() throws Exception {
    ...
    if (difficulty1.compareTo(difficulty2) > 0) {
      ...
      Mockito.verify(ctx2.syncBlockPeerService, Mockito.times(0))
          .getBlockHistory(ArgumentMatchers.any());
    }
  }
  ...
  public void step_4_replace_2_generated() throws Exception {
    ...
    if (difficulty1.compareTo(difficulty2) > 0) {
      Mockito.verify(ctx1.syncBlockPeerService, Mockito.times(1)).getLastBlock();
      Mockito.verify(ctx1.syncBlockPeerService, Mockito.times(2))
          .getBlockHistory(ArgumentMatchers.any());
    } else {
      Mockito.verify(ctx2.syncBlockPeerService, Mockito.times(1)).getLastBlock();
      Mockito.verify(ctx2.syncBlockPeerService, Mockito.times(2))
          .getBlockHistory(ArgumentMatchers.any());
    }
  }
  ...
  public void step_5_too_many_blocks() throws Exception {
    ...
    int time = lastBlock.getTimestamp() + Constant.SECONDS_IN_DAY * 2 + 1;
    ...
    Mockito.verify(ctx1.syncBlockPeerService, Mockito.times(1)).getLastBlock();
    Mockito.verify(ctx1.syncBlockPeerService, Mockito.atLeast(2))
        .getBlockHistory(ArgumentMatchers.any());
  ...
  public void step_6_sync_with_secondary_blockchain() throws Exception {
    ...
    Assert.assertTrue(
        "ctx2Chain generated better block",
    ...
    while (lastBlock1.getCumulativeDifficulty().compareTo(lastBlock2.getCumulativeDifficulty())
        < 0) {
  ...
  public void step_7_set_pointer_to_primary_blockchain() throws Exception {
    ...
    Transaction tx1 =
        PaymentBuilder.createNew(100L, new AccountID(publicKey))
    ...
    Mockito.when(mockLedgerProvider.getLedger(ArgumentMatchers.any()))
        .thenAnswer(
            new Answer<Object>() {
              @Override
    ...
    Mockito.when(mockLedgerProvider.addLedger(ArgumentMatchers.any()))
        .thenAnswer(
            new Answer<Object>() {
              @Override
              public Object answer(InvocationOnMock invocationOnMock) throws Throwable {
