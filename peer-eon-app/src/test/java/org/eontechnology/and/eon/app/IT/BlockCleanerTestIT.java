...
public class BlockCleanerTestIT {
  ...
  private static int LIMIT_BLOCK_COUNT = Constant.STORAGE_FRAME_AGE / 180 + 10;
  ...
  public void step_1_cleaning() throws Exception {
    ...
    for (int i = 0; i < BLOCK_COUNT; i++) {
      ...
      Transaction tx1 =
          PaymentBuilder.createNew(10000L, recipient2)
              .validity(lastBlock.getTimestamp() + 100, 3600)
              .build(ctx1.getNetworkID(), ctx1.getSigner());
      Transaction tx2 =
          PaymentBuilder.createNew(10000L, recipient1)
              .validity(lastBlock.getTimestamp() + 100, 3600)
              .build(ctx1.getNetworkID(), ctx1.getSigner());
    ...
    Dao<DbTransaction, Long> txDao1 =
        DaoManager.createDao(ctx1.storage.getConnectionSource(), DbTransaction.class);
    Dao<DbTransaction, Long> txDao2 =
        DaoManager.createDao(ctx2.storage.getConnectionSource(), DbTransaction.class);
    Dao<DbBlock, Long> blockDao1 =
        DaoManager.createDao(ctx1.storage.getConnectionSource(), DbBlock.class);
    Dao<DbBlock, Long> blockDao2 =
        DaoManager.createDao(ctx2.storage.getConnectionSource(), DbBlock.class);
    ...
    Assert.assertEquals(
        ...
        ctx2.blockExplorerService.getLastBlock().getID());
    Assert.assertEquals(
        "Blockchain synchronized and not empty",
    ...
    Assert.assertEquals(Constant.STORAGE_FRAME_AGE / 180, blockDao2.countOf() - 3);
  }
}
