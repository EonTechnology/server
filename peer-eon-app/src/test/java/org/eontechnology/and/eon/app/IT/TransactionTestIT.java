...
public class TransactionTestIT {
  ...
  public void step_1_doubleSending() throws Exception {
    ...
    Transaction tx2 =
        PaymentBuilder.createNew(10000L, new AccountID(signer.getPublicKey()))
    ...
    Transaction tx3 =
        PaymentBuilder.createNew(8000L, new AccountID(ctx.getSigner().getPublicKey()))
            .forFee(1000L)
            .validity(lastBlock.getTimestamp() + 200, 3600)
            .build(ctx.getNetworkID(), signer);
    Transaction tx4 =
        PaymentBuilder.createNew(8000L, new AccountID(ctx2.getSigner().getPublicKey()))
            .forFee(1000L)
            .validity(lastBlock.getTimestamp() + 200, 3600)
    ...
    Assert.assertEquals(
        "Block TIMESTAMP equals", lastBlock1.getTimestamp(), lastBlock2.getTimestamp());
  ...
  public void step_2_balances_checker() throws Exception {
    ...
    Assert.assertEquals(
        AccountBotService.State.NotFound, ctx.accountBotService.getState(signerNewID));
    Assert.assertEquals(AccountBotService.State.Unauthorized, balanceNew.state);

    Transaction tx =
        RegistrationBuilder.createNew(signerNew.getPublicKey())
            .validity(lastBlock.getTimestamp() + 100, 3600)
            .build(ctx.getNetworkID(), signer);
    Mockito.when(mockTimeProvider.get()).thenReturn(lastBlock.getTimestamp() + 180 + 1);
    ctx.transactionBotService.putTransaction(tx);
    ...
    Assert.assertEquals(
        AccountBotService.State.Processing, ctx.accountBotService.getState(signerNewID));
    Assert.assertEquals(AccountBotService.State.Unauthorized, balanceNew.state);
    ...
    Transaction tx2 =
        PaymentBuilder.createNew(10000L, new AccountID(signerNew.getPublicKey()))
            .validity(lastBlock.getTimestamp() + 200, 3600)
            .build(ctx.getNetworkID(), signer);
    Mockito.when(mockTimeProvider.get()).thenReturn(lastBlock.getTimestamp() + 180 * 2 + 1);
    ctx.transactionBotService.putTransaction(tx2);
    ...
    Assert.assertTrue(
        "Deposit in sender", ctx.accountBotService.getInformation(signerNewID).deposit == 100L);
    ...
    Assert.assertEquals(
        ...
        ctx.accountBotService.getBalance(signerNewID).amount);
    Assert.assertTrue(
        "Deposit in sender", ctx.accountBotService.getInformation(signerNewID).deposit == 50L);
  }
  ...
  public void step_3_transaction_duplicate() throws Exception {
    ...
    Transaction tx1 =
        PaymentBuilder.createNew(100L, new AccountID(signerNew.getPublicKey()))
            .validity(mockTimeProvider.get(), 3600)
            .build(ctx.getNetworkID(), ctx.signer);
    ...
    ctx.generateBlockForNow();
    Assert.assertEquals(
        "Emprty block", 0, ctx.blockExplorerService.getLastBlock().getTransactions().size());
  }
  ...
  public void step_4_double_spending() throws Exception {
    ...
    // back payment
    Transaction tx2 =
        PaymentBuilder.createNew(50L, new AccountID(ctx.signer.getPublicKey()))
            .validity(mockTimeProvider.get(), 3600)
            .build(ctx.getNetworkID(), signerNew);
    ctx.transactionBotService.putTransaction(tx2);
    Transaction tx3 =
        PaymentBuilder.createNew(70L, new AccountID(ctx.signer.getPublicKey()))
            .validity(mockTimeProvider.get(), 3600)
            .build(ctx.getNetworkID(), signerNew);
    try {
      ctx.transactionBotService.putTransaction(tx3);
