...
public class Fork1TestIT {
  ...
  protected static int BEGIN = 180 * BEGIN_H;
  protected static int END = 180 * END_H;
  protected static int END2 = 180 * END_H2;
  ...
  public void step_1_3_getNumber() throws Exception {
    ...
    Assert.assertEquals(
        "Before fork", 1, forkState1.getNumber(lastBlock.getTimestamp() + BEGIN - 50));
  ...
  public void step_2_1_CheckGeneratorIsDisabled() throws Exception {
    ...
    do {
      ...
      if (!lastBlock.getID().equals(lastBlockID)) {
        ...
        Assert.assertTrue(forkState1.isPassed(lastBlock.getTimestamp() + 180));
      }
    } while (!lastBlockID.equals(lastBlock.getID()));

    Assert.assertEquals(
        "Blockchain max possible height",
        END / 180,
  ...
  public void step_2_2_CheckGeneratorIsDisabled() throws Exception {
    ...
    do {
      ...
      if (!lastBlock.getID().equals(lastBlockID)) {
        ...
        Assert.assertTrue(forkState2.isPassed(lastBlock.getTimestamp() + 180));
      }
    } while (!lastBlockID.equals(lastBlock.getID()));
  ...
  public void step_3_2_CheckBlockSyncIsDisabled() throws Exception {
    ...
    Assert.assertEquals(
        "Blockchain synchronized to possible height", END / 180, lastBlock1.getHeight());
    Assert.assertEquals("Blockchain max possible height", END2 / 180, lastBlock2.getHeight());
  }
  ...
  public void step_4_CheckTranSyncIsDisabled() throws Exception {
    ...
    do {
      ...
      Transaction tx1 =
          PaymentBuilder.createNew(100L, new AccountID(ctx2.getSigner().getPublicKey()))
  ...
  public void step_6_tran_version_checked() throws Exception {
    ...
    time = lastBlock.getTimestamp() + BEGIN + 180 + 1;
    Mockito.when(mockTimeProvider.get()).thenReturn(time);
