...
public class GenerationTestIT {
  ...
  private static final String GENERATOR2 =
      "d2005ef0df1f6926082aefa09917874cfb212d1ff4eb55c78f670ef9dd23ef6c";
  private static final String GENERATOR_NEW =
  ...
  public void step_1_generate_block_early() throws Exception {
    ...
    PeerContext ctx =
        new PeerContext(PeerStarterFactory.create().seed(GENERATOR).build(mockTimeProvider));
  ...
  public void step_2_generate_block_not_started() throws Exception {
    ...
    Assert.assertFalse(
        "Generation should be not allowed till blockchain sync",
  ...
  public void step_3_generate_block() throws Exception {
    ...
    Assert.assertEquals(
        "Period is 180 sec", lastBlock.getTimestamp() + 180, generatedBlock.getTimestamp());
  }
  ...
  public void step_4_generate_2_block() throws Exception {
    ...
    Assert.assertEquals("DB store new block ID", generatedBlock.getID(), newLastBlock.getID());
    Assert.assertEquals(
        "Period is 180 sec", lastBlock.getTimestamp() + 180 * 2, generatedBlock.getTimestamp());
  }
  ...
  public void step_5_SyncBlockService() throws Exception {
    ...
    Block[] lastBlocks =
        ctx.syncBlockPeerService.getBlockHistory(new String[] {lastBlock.getID().toString()});
  ...
  public void step_7_generate_by_new_acc() throws Exception {
    ...
    PeerContext ctx =
        new PeerContext(PeerStarterFactory.create().seed(GENERATOR_NEW).build(mockTimeProvider));
  ...
  public void step_8_generate_new_acc() throws Exception {
    ...
    Transaction tx =
        RegistrationBuilder.createNew(ctxNew.getSigner().getPublicKey())
    ...
    Transaction tx2 =
        PaymentBuilder.createNew(
                Utils.MIN_DEPOSIT_SIZE + 1000L, new AccountID(ctxNew.getSigner().getPublicKey()))
    ...
    Transaction tx3 =
        DepositBuilder.createNew(Utils.MIN_DEPOSIT_SIZE)
            .validity(lastBlock.getTimestamp() + 200, 3600)
    ...
    Assert.assertEquals(
        "Deposit.refill in block",
        1,
        ctx.blockExplorerService.getLastBlock().getTransactions().size());

    int newTime = lastBlock.getTimestamp() + Constant.SECONDS_IN_DAY + 180 + 1;
    ...
    Assert.assertEquals(
        "Is new block synchronized",
        ctx.blockExplorerService.getLastBlock().getID(),
        ctxNew.blockExplorerService.getLastBlock().getID());

    Transaction tx4 =
        DepositBuilder.createNew(Utils.MIN_DEPOSIT_SIZE + 100L)
    ...
    Assert.assertEquals(
        "Is generated generation stopped",
        ctx.blockExplorerService.getLastBlock().getID(),
        ctxNew.blockExplorerService.getLastBlock().getID());
  ...
  public void step_9_GeneratorReplaceBlock() throws Exception {
    ...
    PeerContext ctx2 =
        new PeerContext(PeerStarterFactory.create().seed(GENERATOR2).build(mockTimeProvider));
  ...
  public void step10_generator_should_use_trs_of_parallel_block() throws Exception {
    ...
    Mockito.when(mockTimeProvider.get()).thenReturn(netStart + 180 + 1);
    ctx1.generateBlockForNow();
    ...
    Transaction tx1 =
        PaymentBuilder.createNew(10000L, ctx_signer_id)
            .validity(netStart + 180 + 2, 3600)
  ...
  public void step_11_generate_with_conflicting_transaction_order() throws Exception {
    ...
    Transaction tx =
        RegistrationBuilder.createNew(newSigner.getPublicKey())
            .validity(mockTimeProvider.get(), 3600)
            .build(ctx.getNetworkID(), ctx.getSigner());
    Transaction tx1 =
        PaymentBuilder.createNew(100L, new AccountID(newSigner.getPublicKey()))
    ...
    Assert.assertEquals(
        "Some transactions are not added to the block", newLastBlock.getTransactions().size(), 1);
  }
}
