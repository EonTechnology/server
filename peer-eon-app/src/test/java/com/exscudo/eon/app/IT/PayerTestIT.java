...
public class PayerTestIT {
    ...
    public void step_1() throws Exception {
        ...
        // check tx history
        Assert.assertEquals("Delegate1 tx count",
                            3,
                            ctx2.transactionExplorerService.getByAccountId(delegate_1_id, 0).size());

        Assert.assertEquals("Delegate2 tx count",
                            2,
                            ctx2.transactionExplorerService.getByAccountId(delegate_2_id, 0).size());

        Assert.assertEquals("Sender tx count", 2, ctx2.transactionExplorerService.getByAccountId(senderID, 0).size());
    }
}
