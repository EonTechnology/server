...
public class ColoredCoinsTestIT {
    ...
    private void create_colored_coin() throws Exception {
        ...
        Transaction tx2 =
                PaymentBuilder.createNew(1000000L, accountID).validity(timestamp, 3600).build(ctx.getSigner());
        ...
        Transaction tx3 =
                ColoredCoinRegistrationBuilder.createNew(10000L, 1).validity(timestamp, 3600).build(coloredCoinSigner);
    ...
    public void step_3_change_money_supply() throws Exception {
        ...
        timestamp = mockTimeProvider.get();
        Transaction tx1 =
                ColoredCoinSupplyBuilder.createNew(11000L).validity(timestamp, 60 * 60).build(coloredCoinSigner);
        ctx.transactionBotService.putTransaction(tx1);
        Mockito.when(mockTimeProvider.get()).thenReturn(timestamp + 180);
        ...
        timestamp = mockTimeProvider.get();
        Transaction tx2 =
                ColoredCoinSupplyBuilder.createNew(3000L).validity(timestamp, 60 * 60).build(coloredCoinSigner);
        try {
            ctx.transactionBotService.putTransaction(tx2);
        ...
        timestamp = mockTimeProvider.get();
        Transaction tx4 =
                ColoredCoinSupplyBuilder.createNew(4000L).validity(timestamp, 60 * 60).build(coloredCoinSigner);
