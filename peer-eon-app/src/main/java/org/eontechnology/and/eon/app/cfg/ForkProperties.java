...
public class ForkProperties {
  ...
  public static ForkProperties parse(URI uri) throws IOException {
    ...
    allowedParams.add("block_period");
    allowedParams.add("block_size");
    allowedParams.add("generation_salt_version");

    ObjectMapper objectMapper = new ObjectMapper();
    byte[] json = Files.readAllBytes(Paths.get(uri));
    Map<String, Object> forksMap =
    ...
    for (Map<String, Object> map : periodJson) {
      ...
      if (map.containsKey("params")) {
        @SuppressWarnings("unchecked")
        Map<String, Number> params = (Map<String, Number>) map.get("params");
        if (params.size() > 0) {
          p.setParams(params);
        }
      ...
      size += map.containsKey("params") ? 1 : 0;
